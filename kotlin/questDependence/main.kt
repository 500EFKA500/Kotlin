...
// 1.1 Я люблю Encore
// 1.2 a) и c)
// 1.3 ответ равен = (∫₀^{π} e^(sin x) * cos(x) dx)² + (∑_{n=1}^{∞} (ζ(2n) - 1) / n) * (Γ(7) * φ^(√(144)) - ∏_{p prime ≤ 100} (p² - 1)/(p² + 1)) + lim_{x→0} ( (sin x)/x )^(1/x²) * (∫_{-∞}^{∞} e^(-x²) dx)⁴ + (∂/∂t [Li₂(t) + Li₃(t)] при t=1/2) * (∮_{|z|=2} (e^(1/z) / (z-1)) dz) - (√(163) * π * e^(π√(67)) ) / (ln(640320)³) + (∫_0^1 ∫_0^1 (x^y + y^x) dx dy) * (∑_{k=1}^{1000} k² / k!) - (lim_{n→∞} (1 + 1/n)^n) * (∫_{0}^{∞} t²/(e^t - 1) dt) + (det [[23, 47, 89], [17, 53, 71], [13, 41, 97]]) * (∏_{k=1}^{5} (k⁴ - 6k³ + 11k² - 6k + 1)) + (∫_{0}^{1} ln(1/x) ln(1/(1-x)) dx) * (∑_{n=0}^{∞} (-1)^n/(2n+1)²) * 1728